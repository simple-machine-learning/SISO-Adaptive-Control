...
from project_setup import (
    ...
    ctrl_lnu_lnu_file as ctrl_file,
    ...
    r_0_init,
    alpha_v, alpha_r_0,
    bibs_ctrl_lnu_lnu_file as bibs_ctrl_file,
    reference_type,
    reference_seed,
    d_min,
    d_max,
...
def closed_loop_characteristic_metrics(w, v, r_0_value, n_y, n_u, n_u1, n_M):
    # Plant: y(k) = w_y*[y(k-1)...] + w_u*[u(k-n_u1-1)...]
    # Controller in this script:
    # q(s) = v[0] + v_y*[y(s),...,y(s-n_y+1)]
    #              + v_e*[e_ref(s),...,e_ref(s-n_e+1)]
    # Homogeneous stability test: d=0 and y_ref=0, therefore e_ref=y.
    # Hence v_cl contains the effective feedback coefficients from y to q.
    w_y = w[1:1+n_y]
...
for epoch in range(epochs):
    ...
    for k in range(N_start, N):
        ...
        if learning == "NGD":
            eta_v = mu_v/(eps + g_v @ g_v)
            eta_r_0 = mu_r_0/(eps + g_r_0*g_r_0)
            dv = -eta_v*e_ref[k]*g_v
            dr_0 = -eta_r_0*e_ref[k]*g_r_0
        elif learning == "GD":
            eta_v = mu_v
            eta_r_0 = mu_r_0
            dv = -eta_v*e_ref[k]*g_v
        ...
        # BIBS monitoring of local controller weight-update dynamics
        # e_ref(k) is locally linearized as e_ref(k) = g_v(k)^T v + forced terms.
        # Then v(k+1) = [I - eta_v(k) g_v(k) g_v(k)^T] v(k) + forced terms.
        A_v = eye(n_v) - eta_v*outer(g_v, g_v)
        A_r_0 = 1.0 - eta_r_0*g_r_0*g_r_0
        A2_v_all[k_all] = linalg.norm(A_v, 2)
        Rho_v_all[k_all] = max(abs(linalg.eigvals(A_v)))
        A_abs_r_0_all[k_all] = abs(A_r_0)
        ...
        m_norm_all[k_all], Rho_all[k_all] = closed_loop_characteristic_metrics(
            w, v, r_0_value, n_y, n_u, n_u1, n_M
        )
    ...
    print(f"Controller LNU-LNU epoch {epoch+1}/{epochs}: RMSE={rmse:.6g}, r_0={r_0_value:.6g}", flush=True)
...
fig.suptitle(
    "Measured dataset" + "\n" + r"LNU plant + LNU controller, learning=" + learning +
    r", $\mu_v$=" + str(mu_v) +
