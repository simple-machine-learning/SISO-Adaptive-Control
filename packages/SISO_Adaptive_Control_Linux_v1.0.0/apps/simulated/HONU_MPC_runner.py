...
try:
    from honu_native import native_enabled as honu_native_enabled, predict_sequence_and_jacobian as native_honu_predict, optimize_u as native_honu_optimize
except ImportError:
    honu_native_enabled=lambda: False; native_honu_predict=None; native_honu_optimize=None
try:
    from mlp_native import native_enabled as mlp_native_enabled, predict_sequence_and_jacobian as native_mlp_predict
except ImportError:
    mlp_native_enabled=lambda: False; native_mlp_predict=None
try:
    from direct_native import native_enabled as direct_native_enabled, predict_sequence_and_jacobian as native_direct_predict
except ImportError:
    direct_native_enabled=lambda: False; native_direct_predict=None
...
def predict_sequence_and_jacobian(candidate_u, y_hist, u_hist, local, compute_jacobian=True):
    ...
    if local.get("prediction_mode") == "recursive" and local.get("model") == "MLP":
        if not mlp_native_enabled() or native_mlp_predict is None:
            raise RuntimeError(
                "C++ MLP predictor is required for recursive MLP mode, but it is unavailable or disabled. "
                "Python fallback is forbidden. Run ./build_native_linux.sh."
            )
        return native_mlp_predict(candidate_u, y_hist, u_hist, local, compute_jacobian=compute_jacobian)
    if local.get("prediction_mode") == "recursive" and local.get("model") in {"LNU", "QNU"}:
        if not honu_native_enabled() or native_honu_predict is None:
            raise RuntimeError(
                "C++ HONU predictor is required for recursive LNU/QNU mode, but it is unavailable or disabled. "
                "Python fallback is forbidden. Run ./build_native_linux.sh."
            )
        return native_honu_predict(candidate_u, y_hist, u_hist, local, compute_jacobian=compute_jacobian)
    if local.get("prediction_mode") == "direct_multi_horizon":
        if not direct_native_enabled() or native_direct_predict is None:
            raise RuntimeError(
                "Cython direct multi-horizon predictor is required, but it is unavailable or disabled. "
                "Python fallback is forbidden. Run ./build_native_linux.sh."
            )
        out, jac = native_direct_predict(candidate_u, y_hist, u_hist, local, compute_jacobian=compute_jacobian)
        h = candidate_u.size
...
def optimize_u(ref, y_hist, u_hist, local, warm, cfg):
    ...
    """
    if local.get("prediction_mode") == "recursive" and local.get("model") in {"LNU", "QNU"}:
        if _HONU_NATIVE_FAILED:
            raise RuntimeError(
                "C++ HONU MPC optimizer failed earlier in this run. Python fallback is forbidden."
            )
        if not honu_native_enabled() or native_honu_optimize is None:
            raise RuntimeError(
                "C++ HONU MPC optimizer is required for recursive LNU/QNU mode, but it is unavailable or disabled. "
                "Python fallback is forbidden. Run ./build_native_linux.sh."
            )
        try:
            return native_honu_optimize(ref, y_hist, u_hist, local, warm, cfg)
        except (RuntimeError, ValueError, OverflowError, FloatingPointError) as exc:
            _HONU_NATIVE_FAILED = True
            raise RuntimeError(
                "C++ HONU MPC optimizer failed. Python fallback is forbidden."
            ) from exc
