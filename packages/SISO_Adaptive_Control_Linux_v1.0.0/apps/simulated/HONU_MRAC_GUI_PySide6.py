# -*- coding: utf-8 -*-
"""
PySide6 + pyqtgraph GUI for HONU MRAC examples.
...
Design rules:
    ...
    - The GUI exposes the complete simulated-plant MRAC setup shown in the top panel.
...
from model_recommended_presets import recommended_preset
...
from shared_plant_model import plant_display_name, plant_signal_metadata, plant_signal_symbol
...
PHYSICAL_PLANT_MODELS = {
    "Bioprocess: dissolved oxygen bioreactor": "bioreactor_dissolved_oxygen",
    "Bioprocess: photobioreactor pH / CO2": "photobioreactor_ph_co2",
    "Bioprocess: Monod chemostat biomass": "chemostat_monod_biomass",
    "Biomedical: nonlinear drug PK": "drug_infusion_pk",
    "Biomedical: nonlinear drug PK-PD": "drug_infusion_pkpd",
    "Biomedical: glucose-insulin model": "glucose_insulin_bergman",
    "Thermal: asymmetric Peltier system": "peltier_thermal_asymmetric",
    "IT: CPU thermal control by fan": "cpu_thermal_fan",
    "IT: cloud-server workload": "cloud_server_workload",
    "IT: cloud-server workload with delay": "cloud_server_workload_with_delay",
    "Network: nonlinear router fluid queue": "network_router_fluid_queue",
    "Network: router fluid queue with delay": "network_router_fluid_queue_with_delay",
    "Network: router fluid queue with large delay": "network_router_fluid_queue_with_large_delay",
    "Wireless: transmit-power / SNR control": "wireless_power_snr",
    "Process: bidirectional-pump nonlinear tank": "bidirectional_tank_level",
    "Mechanical: two masses, viscous damping": "two_mass_actuator_grounded_m2_linear_viscous",
    "Mechanical: two masses, LuGre friction": "two_mass_actuator_grounded_m2_lugre",
    "Mechanical: two masses, LuGre friction 2": "two_mass_actuator_grounded_m2_lugre2",
    "Mechanical: tuned mass vibration absorber": "mechanical_tuned_mass_damper",
    "Mechanical: voice-coil servo": "voice_coil_servo",
    "Mechanical: overhead crane payload sway": "overhead_crane_payload_sway",
    "Mechanical: overhead crane payload sway with delay": "overhead_crane_payload_sway_with_delay",
    "Drone: quadrotor altitude": "quadrotor_altitude",
    "Drone: quadrotor roll": "quadrotor_roll",
    "Accelerator: RF cavity amplitude": "accelerator_rf_cavity_amplitude",
    "Accelerator: RF cavity amplitude with delay": "accelerator_rf_cavity_amplitude_with_delay",
    "Accelerator: transverse beam position": "accelerator_beam_position",
    "Power grid: linear microgrid frequency": "microgrid_frequency_linear",
    "Power grid: nonlinear BESS microgrid frequency": "microgrid_frequency_bess_nonlinear",
    "Power grid: nonlinear BESS microgrid frequency with delay": "microgrid_frequency_bess_nonlinear_with_delay",
    "Soil ecology: moisture-controlled respiration": "soil_moisture_respiration",
    "Soil ecology: moisture respiration with delay": "soil_moisture_respiration_with_delay",
    "Soil ecology: microbial carbon respiration": "soil_microbe_carbon_respiration",
    "Soil ecology: carbon priming": "soil_carbon_priming",
    "Soil ecology: carbon priming with delay": "soil_carbon_priming_with_delay",
    "Soil ecology: nitrogen transformations and N2O": "soil_nitrogen_n2o",
    "Soil ecology: denitrification controlled by aeration": "soil_denitrification_aeration",
    "Soil ecology: microbial C-N stoichiometry": "soil_microbe_cn_stoichiometry",
}
...
SCRIPT_MODULES = {
    "01 Generate physical plant data": "01_generate_plant_data.py",
    ...
    "04 Test LNU-LNU controller on ODE plant": "04_test_ODE_plant_LNU_controller_LNU.py",
    "04 Test LNU-QNU controller on ODE plant": "04_test_ODE_plant_LNU_controller_QNU.py",
    "04 Test QNU-LNU controller on ODE plant": "04_test_ODE_plant_QNU_controller_LNU.py",
    "04 Test QNU-QNU controller on ODE plant": "04_test_ODE_plant_QNU_controller_QNU.py",
}
...
EVALUATION_SCRIPT_BY_METADATA = {
    ("LNU", "LNU"): "04_test_ODE_plant_LNU_controller_LNU.py",
    ("LNU", "QNU"): "04_test_ODE_plant_LNU_controller_QNU.py",
    ("QNU", "LNU"): "04_test_ODE_plant_QNU_controller_LNU.py",
    ("QNU", "QNU"): "04_test_ODE_plant_QNU_controller_QNU.py",
}
...
OUTPUT_FILES = {
    ...
    "04 physical ODE test LNU-LNU": "eval_LNU_LNU_physical.txt",
    "04 physical ODE test LNU-QNU": "eval_LNU_QNU_physical.txt",
    "04 physical ODE test QNU-LNU": "eval_QNU_LNU_physical.txt",
    "04 physical ODE test QNU-QNU": "eval_QNU_QNU_physical.txt",
}
...
class HONUMPCPage(QWidget):
    ...
    def _result_metadata_from_npz(self, data):
        ...
        if metadata.get("dt_sim") is None:
            if "t_sim" in data:
                metadata["dt_sim"] = self._series_dt(data["t_sim"])
            if metadata.get("dt_sim") is None:
                metadata["dt_sim"] = metadata.get("dt_control")
        return metadata
    ...
    def _build_ui(self):
        ...
        for label, value in sorted(PHYSICAL_PLANT_MODELS.items(), key=lambda item: item[0].casefold()): self.physical_model_combo.addItem(label, value)
        self.honu_combo = QComboBox(); self.honu_combo.addItems(["LNU", "QNU", "MLP"]); self.honu_combo.setFixedWidth(82)
        self.prediction_mode_combo=QComboBox(); self.prediction_mode_combo.setFixedWidth(220)
        self.prediction_target_combo=QComboBox(); self.prediction_target_combo.addItem("Absolute output", "absolute"); self.prediction_target_combo.addItem("Output increment", "delta"); self.prediction_target_combo.setFixedWidth(150)
        ...
        self.mpc_excitation_combo.setToolTip("Selects the step sequence for simulation input u, initial MPC identification input u, and MPC reference d.")
        self.dt_control = self._dspin(1e-4, 1e4, 1.0, 0.1, width=72)
        self.dt_sim = self._dspin(1e-5, 1e3, 0.1, 0.01, width=72)
        self.ode_solver_combo = QComboBox()
        for label, value in (("Auto (C++/Numba/SciPy)", "auto"), ("Native RK4 (C++/Numba)", "RK4"), ("Radau (SciPy)", "Radau"), ("BDF (SciPy)", "BDF"), ("LSODA (SciPy)", "LSODA"), ("RK45 (SciPy)", "RK45"), ("DOP853 (SciPy)", "DOP853")):
            self.ode_solver_combo.addItem(label, value)
        self.ode_solver_combo.setFixedWidth(205)
        self.ode_solver_combo.setToolTip("ODE solver for the selected physical model. Auto selects the accelerated native RK4 path (C++/Numba) or a SciPy solver according to the model.")
        self.horizon = self._ispin(1, 500, 12, 64)
        self.window = self._dspin(1e-4, 1e7, 150.0, 1.0, 6, width=78)
        ...
        self.mlp_hidden_layers.setToolTip("Hidden-layer widths for MLP, comma-separated. Examples: 8 or 16,8. Output width is automatic: 1 for recursive prediction and Np for direct multi-horizon prediction.")
        self.mlp_hidden_layers.textChanged.connect(self._mark_mpc_result_stale)
        ...
        self.u_min.setToolTip("Minimum u used for physical ODE simulation and the initial MPC identification phase. MPC control u is unrestricted afterward.")
        self.u_max.setToolTip("Maximum u used for physical ODE simulation and the initial MPC identification phase. MPC control u is unrestricted afterward.")
        ...
        self.window.setToolTip("Sliding-window duration [s]. The effective sample count is computed automatically as ceil(window length / dt MPC), with a minimum imposed by n_y and n_u. The initial excitation lasts for this window duration.")
        self.duration.setToolTip("Duration [s] used by physical ODE simulation and batch HONU identification.")
        self.d_duration.setToolTip("Duration [s] of reference d and the closed-loop MPC runs 3.1 and 3.2.")

        self.simulate_btn = QPushButton("Simulate ODE"); self.simulate_btn.clicked.connect(self.run_simulation_only)
        self.identify_btn = QPushButton("Identify Plant Model"); self.identify_btn.clicked.connect(self.run_identify_honu)
        self.simulate_btn.setToolTip("Simulate the selected physical ODE model without HONU identification or MPC.")
        ...
        self.prediction_mode_combo.currentIndexChanged.connect(self._on_mpc_prediction_changed)
        self.plant_learning_combo.currentIndexChanged.connect(self._mark_mpc_result_stale)
        self.physical_model_combo.currentIndexChanged.connect(self._on_mpc_physical_model_changed)
        ...
        self._on_mpc_model_changed(self.honu_combo.currentText())
        ...
        self.simulate_btn.setText("1. Simulate physical ODE plant")
        self.identify_btn.setText("2. Identify Plant Model")
        self.run_frozen_btn.setText("3.1 MPC - Frozen Model")
        ...
        self.btn_result_simulation = QPushButton("1. Physical ODE Plant Simulation")
        ...
        top_outer.addWidget(self._labeled("physical plant", self.physical_model_combo, 420))
        top_outer.addWidget(self._labeled("HONU plant", self.honu_combo))
        top_outer.addWidget(self._labeled("prediction", self.prediction_mode_combo))
        top_outer.addWidget(self._labeled("target", self.prediction_target_combo))
        ...
        add_param_row(0, "Simulation", [
            ("t_sim [s]", self.duration),
            ("u step width [s]", self.excitation_hold),
            ("tau_u [s]", self.tau_u_delay),
            ("dt MPC [s]", self.dt_control),
            ("dt sim [s]", self.dt_sim),
            ("ODE solver", self.ode_solver_combo),
            ("u_min", self.u_min),
            ("u_max", self.u_max),
            ("P regulator", self.preg_enabled_check),
            ("r_Preg", self.r_preg),
        ])
        add_param_row(1, "Reference d", [
        ...
        self._on_mpc_physical_model_changed()

    def _on_mpc_physical_model_changed(self, *_args):
        """Apply model-specific MPC defaults instead of retaining generic values."""
        model_name = self.physical_model_combo.currentData()
        preset = recommended_preset(model_name) or {}
        common = (
            (self.dt_control, "dt"),
            (self.dt_sim, "dt_sim"),
            (self.duration, "t_end"),
            (self.d_duration, "reference_duration_sec"),
            (self.excitation_hold, "step_hold_sec"),
            (self.ref_hold, "reference_step_hold_sec"),
            (self.tau_u_delay, "tau_u"),
            (self.tau_d_delay, "tau_d"),
            (self.tau1, "Tau_1"),
            (self.tau2, "Tau_2"),
            (self.u_min, "u_min"),
            (self.u_max, "u_max"),
        ...
        for widget, key in common:
            if key in preset:
                widget.setValue(preset[key])
        self.preg_enabled_check.setChecked(bool(preset.get("preg_blackbox_enabled", False)))

        # MPC parameters are controller-specific and therefore are not copied
        # from the MRAC controller-learning settings.
        mpc_defaults = {
            "quadrotor_altitude": dict(duration=120.0, d_duration=120.0, window=20.0, ridge=0.05,
                horizon=30, q_track=35.0, r_du=25.0, r_ddu=50.0, r_u=0.10,
                mu_bibs=0.25, opt_iter=25),
            "quadrotor_roll": dict(duration=60.0, d_duration=60.0, window=10.0, ridge=0.05,
                horizon=25, q_track=35.0, r_du=20.0, r_ddu=40.0, r_u=0.08,
                mu_bibs=0.25, opt_iter=25),
        }.get(str(model_name), {})
        for name, value in mpc_defaults.items():
            getattr(self, name).setValue(value)
        self.status.append(
            f"MPC MODEL PRESET: {model_name}; dt MPC={self.dt_control.value():g} s, "
            f"dt sim={self.dt_sim.value():g} s, u=[{self.u_min.value():g}, {self.u_max.value():g}], "
            f"d=[{self.d_min.value():g}, {self.d_max.value():g}]"
        )
    ...
    def open_current_graph_full_screen(self):
        ...
        for key, label in (("dt_control", "dt MPC"), ("dt_sim", "dt sim")):
            try:
    ...
    def _mark_mpc_result_stale(self, *_args):
        # Changing a structural model setting does not alter the already loaded
        # result file. Make this explicit so a previous recursive graph is not
        # mistaken for a newly selected direct model (or vice versa).
        if hasattr(self, "status_label") and self.current_result_mode:
            self.status_label.setText("Settings changed — displayed graph is from the previous run")
    ...
    def _on_mpc_model_changed(self, *_args):
        ...
        previous = self.prediction_mode_combo.currentData()
        self.prediction_mode_combo.blockSignals(True)
        self.prediction_mode_combo.clear()
        if model == "MLP":
            self.prediction_mode_combo.addItem("Recursive one-step", "recursive")
            self.prediction_mode_combo.addItem("Direct multi-horizon + SLSQP", "direct_multi_horizon")
        else:
            self.prediction_mode_combo.addItem("Recursive one-step", "recursive")
            self.prediction_mode_combo.addItem("Recursive rollout-trained", "recursive_rollout")
            self.prediction_mode_combo.addItem("Direct multi-horizon", "direct_multi_horizon")
        idx = self.prediction_mode_combo.findData(previous)
        self.prediction_mode_combo.setCurrentIndex(idx if idx >= 0 else 0)
        self.prediction_mode_combo.blockSignals(False)

        is_mlp = model == "MLP"
        previous_learning=self.plant_learning_combo.currentData()
        ...
        if is_mlp:
            ...
            idx=self.plant_learning_combo.findData(previous_learning)
            self.plant_learning_combo.setCurrentIndex(idx if idx>=0 else 2)
        else:
            self.plant_learning_combo.addItem("Ridge", "ridge")
            if self.prediction_mode_combo.currentData() == "recursive":
                self.plant_learning_combo.addItem("L-M", "lm")
            idx=self.plant_learning_combo.findData(previous_learning)
            self.plant_learning_combo.setCurrentIndex(idx if idx>=0 else 0)
        self.plant_learning_combo.blockSignals(False)
        # The supported targets are intentionally fixed per backend:
        # LNU/QNU predict the absolute output; MLP uses a residual increment.
        fixed_target = "delta" if is_mlp else "absolute"
        self.prediction_target_combo.setCurrentIndex(self.prediction_target_combo.findData(fixed_target))
        self.prediction_target_combo.setEnabled(False)
        self.pca_mode.setEnabled(True)
        self.pca_variability.setEnabled(self.pca_mode.currentText() == "Variability")
        self.mlp_hidden_layers.setEnabled(is_mlp)
        self.mu_bibs.setEnabled(not is_mlp)
        self.eps_bibs.setEnabled(not is_mlp)
        self.lm_epochs.setEnabled(True)
        self.lm_epochs.setToolTip("MLP optimizer iterations/epochs." if is_mlp else "Levenberg-Marquardt epochs for each sliding-window HONU fit.")
        ...
        self._on_mpc_learning_changed()
        self._mark_mpc_result_stale()

    def _on_mpc_prediction_changed(self, *_args):
        # L-M is defined only for the one-step recursive HONU identification.
        # Rollout-trained and direct multi-horizon predictors use Ridge.
        if str(self.honu_combo.currentText()).upper() != "MLP":
            previous_learning = self.plant_learning_combo.currentData()
            self.plant_learning_combo.blockSignals(True)
            self.plant_learning_combo.clear()
            self.plant_learning_combo.addItem("Ridge", "ridge")
    ...
    def _config(self):
        ...
        return {"plant_model":self.physical_model_combo.currentData(),"honu":self.honu_combo.currentText(),
                "prediction_mode":self.prediction_mode_combo.currentData(),
                "prediction_target":("delta" if self.honu_combo.currentText().upper()=="MLP" else "absolute"),
                "preg_blackbox_enabled":bool(self.preg_enabled_check.isChecked()),"r_preg":self.r_preg.value(),
                "plant_learning":self.plant_learning_combo.currentData(),"mlp_optimizer":self.plant_learning_combo.currentData(),"lm_epochs":self.lm_epochs.value(),"lambda":self.ridge.value(),
                "dt_control":self.dt_control.value(),"dt_sim":self.dt_sim.value(),"ode_solver":self.ode_solver_combo.currentData(),"horizon":self.horizon.value(),
                "window_length_sec":self.window.value(),"n_y":self.n_y.value(),"n_u":self.n_u.value(),
                ...
                "mlp_hidden_layers":self.mlp_hidden_layers.text().strip(),"mlp_epochs":self.lm_epochs.value(),"mlp_learning_rate":1.0e-3}
    ...
    def _append_mpc_configuration(self, cfg, run_mode):
        ...
        self.status.append(
            f"plant={cfg['plant_model']}; plant_mode={'P-regulated' if cfg['preg_blackbox_enabled'] else 'standalone'}; "
            f"r_Preg={cfg['r_preg']:.9g}; model={cfg['honu']}; prediction={cfg['prediction_mode']}; n_y={cfg['n_y']}; n_u={cfg['n_u']}" + (f"; MLP_hidden={cfg['mlp_hidden_layers']}; output={'Np' if cfg['prediction_mode']=='direct_multi_horizon' else '1'}" if str(cfg['honu']).upper()=='MLP' else '')
        )
        self.status.append(
            f"t_sim={cfg['duration_sec']:.9g} s; d_duration={cfg['reference_duration_sec']:.9g} s; "
            f"dt_MPC={cfg['dt_control']:.9g} s; dt_sim={cfg['dt_sim']:.9g} s; "
            f"ODE_solver={cfg.get('ode_solver', 'auto')}; line_width={self.line_width_spin.value():.9g} px"
        )
    ...
    def run_mpc_frozen(self):
        ...
        The frozen plant model is independent of the subsequently selected
        reference d.  Changing either the duration or the type of d therefore
        does not invalidate an already identified HONU Plant.
        """
    ...
    def run_simulation_only(self):
        self._start_process("simulate")

    def _start_process(self, run_mode):
        if self.process is not None and self.process.state()!=QProcess.NotRunning: return
        if self.dt_sim.value()>self.dt_control.value():
            QMessageBox.warning(self,"HONU MPC","dt sim must not exceed dt MPC."); return
        ...
        if bool(self.preg_enabled_check.isChecked()) and abs(self.r_preg.value()) < 1.0e-15:
            QMessageBox.warning(self,"HONU MPC","ODE + P regulator requires non-zero r_Preg."); return
        ...
        cfg=self._config(); cfg["run_mode"]=run_mode
    ...
    def _read_out(self):
        self.status.append(bytes(self.process.readAllStandardOutput()).decode(errors="replace").rstrip())
    def _read_err(self):
        self.status.append(bytes(self.process.readAllStandardError()).decode(errors="replace").rstrip())
    ...
    def show_simulation_result(self):
        if not self.simulation_output_file.exists():
            self.status.append("No Physical ODE Plant Simulation result is available.")
            return
        self.output_file = self.simulation_output_file
    ...
    def load_result(self, update_log=True, preserve_view=False):
        ...
        dt_sim_value=result_meta.get("dt_sim")
        try:
            dt_sim_value=float(dt_sim_value)
        except (TypeError, ValueError):
            dt_sim_value=dt
        ...
        # Pure ODE simulation has no HONU/BIBS diagnostics. Keep only the
        ...
        if result_mode == "identify":
            ...
            prediction_value=np.asarray(z.get("prediction_mode",["recursive"])).reshape(-1)[0]
            if isinstance(prediction_value,bytes): prediction_value=prediction_value.decode("utf-8",errors="replace")
            direct_ident=str(prediction_value).lower()=="direct_multi_horizon"
            ...
            p0.plot(t, z["y"], pen=pg.mkPen("k", width=lw), name="measured y")
            if direct_ident:
                pred_h=np.asarray(z.get("direct_y_pred",[]),dtype=float)
                err_h=np.asarray(z.get("direct_e_by_horizon",[]),dtype=float)
                H=pred_h.shape[0] if pred_h.ndim==2 else 0
                selected=sorted(set([0,max(0,H//2),max(0,H-1)])) if H else []
                pens=[pg.mkPen("g",width=lw),pg.mkPen("b",width=max(1.0,lw*.9)),pg.mkPen("c",width=max(1.0,lw*.9))]
                for q,j in enumerate(selected):
                    mask=np.isfinite(pred_h[j])
                    p0.plot(t[mask],pred_h[j,mask],pen=pens[min(q,len(pens)-1)],name=f"direct y_hat(k+{j+1})")
                    if err_h.ndim==2 and j<err_h.shape[0]:
                        em=np.isfinite(err_h[j])
                        p2.plot(t[em],err_h[j,em],pen=pg.mkPen("r",width=max(1.0,lw*(1.0-.15*q))),name=f"e_{j+1}")
                p0.setLabel("left", "y, direct predictions")
                p2.setLabel("left", "e_j = y - y_hat_j")
            else:
            ...
            if direct_ident:
                rmse_h=np.asarray(z.get("direct_rmse_by_horizon",[]),dtype=float).reshape(-1)
                rho_med=np.asarray(z.get("direct_rho_median_by_horizon",[]),dtype=float).reshape(-1)
                rho_max=np.asarray(z.get("direct_rho_max_by_horizon",[]),dtype=float).reshape(-1)
                rho_samples=np.asarray(z.get("direct_rho_samples",[]),dtype=float)
                W=np.asarray(z.get("theta",[]),dtype=float)
                model_name=np.asarray(z.get("model",[honu_name]),dtype=object).reshape(-1)[0]
                if isinstance(model_name,bytes): model_name=model_name.decode("utf-8",errors="replace")
                model_name=str(model_name).upper()
                h_axis=np.arange(1,rmse_h.size+1,dtype=float)
                w0.setVisible(rmse_h.size>0); w1.setVisible(W.ndim==2 and W.size>0)
                if rmse_h.size:
                    w0.plot(h_axis,rmse_h,pen=pg.mkPen("r",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw))
                    w0.setLabel("left","RMSE_j")
                    w0.setLabel("bottom","prediction horizon j")
                if W.ndim==2 and W.size:
                    norms=np.linalg.norm(W,axis=0)
                    w1.plot(np.arange(1,norms.size+1,dtype=float),norms,pen=pg.mkPen("k",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw))
                    w1.setLabel("left","||w_j||2")
                    w1.setLabel("bottom","prediction horizon j")
                # Each direct horizon j has its own AR-equivalent A_y,j diagnostic.
                # For LNU there is one rho(A_y,j) per horizon. For QNU the local
                # linearisation depends on the identification sample k, so we show
                # per-horizon summaries over k instead of an unqualified median over
                # different horizons.
                show_rho = rho_med.size>0 or rho_max.size>0
                if model_name == "QNU":
                    r0.setVisible(rho_med.size>0)
                    r1.setVisible(rho_max.size>0)
                    if rho_med.size:
                        xh=np.arange(1,rho_med.size+1,dtype=float)
                        r0.plot(xh,rho_med,pen=pg.mkPen("k",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw),name="median over identification samples")
                        r0.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r0.setLabel("left","median_k rho(A_y,j;k)")
                        r0.setLabel("bottom","prediction horizon j")
                    if rho_max.size:
                        xh=np.arange(1,rho_max.size+1,dtype=float)
                        r1.plot(xh,rho_max,pen=pg.mkPen("r",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw),name="max over identification samples")
                        r1.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r1.setLabel("left","max_k rho(A_y,j;k)")
                        r1.setLabel("bottom","prediction horizon j")
                else:
                    rho_line = rho_med if rho_med.size else rho_max
                    r0.setVisible(rho_line.size>0)
                    r1.setVisible(False)
                    if rho_line.size:
                        xh=np.arange(1,rho_line.size+1,dtype=float)
                        r0.plot(xh,rho_line,pen=pg.mkPen("k",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw),name="rho(A_y,j)")
                        r0.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r0.setLabel("left","rho(A_y,j)")
                        r0.setLabel("bottom","prediction horizon j")
                ...
                self.tab_titles[self.tabs.widget(0)].setText(
                    f'{plant_title} | direct multi-horizon identification of {honu_name} Plant HONU | {learning_name} | {plant_mode_text} | dt MPC = {dt:g} s',
                    color='#183b73', size='12pt', bold=True)
                self.tab_titles[self.tabs.widget(1)].setText(
                    "Direct prediction RMSE and HONU weight norm by horizon",
                    color="#183b73", size="12pt", bold=True)
                if model_name == "QNU":
                    self.tab_titles[self.tabs.widget(2)].setText(
                        "Per-horizon rho(A_y,j): median and max over identification samples",
                        color="#183b73", size="12pt", bold=True)
                else:
                    self.tab_titles[self.tabs.widget(2)].setText(
                        "Per-horizon rho(A_y,j) of the direct HONU models",
                        color="#183b73", size="12pt", bold=True)
            ...
            if not direct_ident:
                ...
                if str(honu_name).upper() == "MLP":
                    learning_name = {"adam":"Adam", "lbfgs":"L-BFGS", "adam_lbfgs":"Adam + L-BFGS"}.get(learning_key, str(learning_key))
                else:
                    learning_name = "L-M" if learning_key == "lm" else "RIDGE"
                recursive_prediction_label = "recursive rollout-trained" if str(prediction_value).lower()=="recursive_rollout" else "recursive one-step"
                self.tab_titles[self.tabs.widget(0)].setText(
                    f'{plant_title} | {recursive_prediction_label} identification of {honu_name} Plant HONU | {learning_name} | {plant_mode_text} | dt MPC = {dt:g} s',
                    color='#183b73', size='12pt', bold=True)
                self.tab_titles[self.tabs.widget(1)].setText(
            ...
            # ODE trajectory at dt_sim and explicitly mark the dt_MPC samples.
            self.tabs.setTabText(0, "ODE simulation")
            self.btn_result_simulation.setText("1. Physical ODE Plant Simulation")
            t_sim = np.asarray(z.get("t_sim", t), dtype=float)
            ...
            p0.plot(t_sim, u_sim, pen=pg.mkPen("b", width=lw), name="u(dt_sim)")
            p0.setLabel("left", "u_new" if preg_enabled else "u")
            self.tab_titles[self.tabs.widget(0)].setText(
                f'{plant_title} | ODE simulation | {plant_mode_text} | top: blue u, bottom: green y with blue samples | dt sim = {dt_sim_value:g} s | dt MPC = {dt:g} s',
                color='#183b73', size='12pt', bold=True)
            p1.plot(t_sim, y_sim, pen=pg.mkPen("g", width=lw), name="y(dt_sim)")
            if t_mpc.size and y_mpc.size:
                point_size = max(1.0, 1.5 * lw)
                point_edge_width = max(0.35, 0.3 * lw)
                p1.plot(t_mpc, y_mpc, pen=None, symbol="o", symbolSize=point_size,
            ...
            p1.setLabel("bottom", f"time [s], dt sim = {dt_sim_value:g} s, dt MPC = {dt:g} s")
            ...
            prediction_value=np.asarray(z.get("prediction_mode",["recursive"])).reshape(-1)[0]
            if isinstance(prediction_value,bytes): prediction_value=prediction_value.decode("utf-8",errors="replace")
            prediction_key=str(prediction_value).lower()
            prediction_label=("Direct multi-horizon" if prediction_key=="direct_multi_horizon" else ("Recursive rollout-trained" if prediction_key=="recursive_rollout" else "Recursive one-step"))
            ...
            self.tab_titles[self.tabs.widget(0)].setText(
                f'{plant_title} | {honu_name} ' + ('batch-trained frozen HONU MPC' if result_mode == 'mpc_frozen' else 'sliding-retraining HONU MPC') + f' | prediction = {prediction_label} | {plant_mode_text} | dt MPC = {dt:g} s | Np = {horizon_text}',
                color='#183b73', size='12pt', bold=True)
            p1.plot(t,z["u"],pen=pg.mkPen("b",width=lw)); p1.setLabel("left","u_new" if preg_enabled else "u")
            p2.plot(t,z["e"],pen=pg.mkPen("r",width=lw)); p2.setLabel("left","e = y_ref - y"); p2.setLabel("bottom",f"time [s], dt MPC = {dt:g} s, dt sim = {dt_sim_value:g} s")
        if result_mode in {"mpc_frozen", "mpc_sliding"}:
            ...
            weights_title += f" | prediction = {prediction_label}"
            ...
            direct_mpc=prediction_key=="direct_multi_horizon"
            rho_bottom_label = f"time [s], dt update = {dt:g} s"
            if direct_mpc and result_mode == "mpc_frozen":
                rho_med=np.asarray(z.get("direct_rho_median_by_horizon",[]),dtype=float).reshape(-1)
                rho_max=np.asarray(z.get("direct_rho_max_by_horizon",[]),dtype=float).reshape(-1)
                model_value=np.asarray(z.get("config_honu",[honu_name])).reshape(-1)[0]
                if isinstance(model_value,bytes): model_value=model_value.decode("utf-8",errors="replace")
                model_value=str(model_value).upper()
                if model_value == "QNU":
                    r0.setVisible(rho_med.size>0); r1.setVisible(rho_max.size>0)
                    if rho_med.size:
                        xh=np.arange(1,rho_med.size+1,dtype=float)
                        r0.plot(xh,rho_med,pen=pg.mkPen("k",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw),name="median over identification samples")
                        r0.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                    ...
                    if rho_max.size:
                        ...
                        r1.setLabel("left","max_k rho(A_y,j;k)")
                        r1.setLabel("bottom","prediction horizon j")
                    rho_title="Frozen direct QNU: per-horizon rho(A_y,j) from plant identification"
                else:
                    rho_line=rho_med if rho_med.size else rho_max
                    r0.setVisible(False); r1.setVisible(rho_line.size>0)
                    if rho_line.size:
                        xh=np.arange(1,rho_line.size+1,dtype=float)
                        r1.plot(xh,rho_line,pen=pg.mkPen("k",width=lw),symbol="o",symbolSize=max(3.0,1.5*lw),name="rho(A_y,j)")
                        r1.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r1.setLabel("left","rho(A_y,j)")
                        r1.setLabel("bottom","prediction horizon j")
                    rho_title="Frozen direct LNU: per-horizon rho(A_y,j) from plant identification"
                self.tab_titles[self.tabs.widget(2)].setText(rho_title + f" | prediction = {prediction_label}",color="#183b73",size="12pt",bold=True)
            elif direct_mpc and result_mode == "mpc_sliding":
                rho_med_trace=np.asarray(z.get("direct_rho_median_trace",[]),dtype=float)
                rho_max_trace=np.asarray(z.get("direct_rho_max_trace",[]),dtype=float)
                model_value=np.asarray(z.get("config_honu",[honu_name])).reshape(-1)[0]
                if isinstance(model_value,bytes): model_value=model_value.decode("utf-8",errors="replace")
                model_value=str(model_value).upper()
                valid_med=rho_med_trace.ndim==2 and rho_med_trace.shape[0]>0 and np.isfinite(rho_med_trace).any()
                valid_max=rho_max_trace.ndim==2 and rho_max_trace.shape[0]>0 and np.isfinite(rho_max_trace).any()
                if model_value == "QNU":
                    r0.setVisible(valid_med); r1.setVisible(valid_max)
                    if valid_med:
                        n_h=min(rho_med_trace.shape[1],int(horizon_text) if str(horizon_text).isdigit() else rho_med_trace.shape[1])
                        for j in range(n_h):
                            mask=np.isfinite(rho_med_trace[:,j])
                            if np.any(mask):
                                color=pg.intColor(j,hues=max(8,n_h),values=1,maxValue=220)
                                r0.plot(t[mask],rho_med_trace[mask,j],pen=pg.mkPen(color,width=max(1.0,lw*.8)),name=f"j={j+1}")
                        r0.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r0.setLabel("left","median_k rho(A_y,j;k)")
                        r0.setLabel("bottom",rho_bottom_label)
                    if valid_max:
                        n_h=rho_max_trace.shape[1]
                        for j in range(n_h):
                            mask=np.isfinite(rho_max_trace[:,j])
                            if np.any(mask):
                                color=pg.intColor(j,hues=max(8,n_h),values=1,maxValue=220)
                                r1.plot(t[mask],rho_max_trace[mask,j],pen=pg.mkPen(color,width=max(1.0,lw*.8)),name=f"j={j+1}")
                        r1.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r1.setLabel("left","max_k rho(A_y,j;k)")
                        r1.setLabel("bottom",rho_bottom_label)
                    rho_title="Sliding direct QNU: per-horizon rho(A_y,j) over retraining time"
                else:
                    rho_trace=rho_med_trace if valid_med else rho_max_trace
                    valid=rho_trace.ndim==2 and np.isfinite(rho_trace).any()
                    r0.setVisible(False); r1.setVisible(valid)
                    if valid:
                        n_h=rho_trace.shape[1]
                        for j in range(n_h):
                            mask=np.isfinite(rho_trace[:,j])
                            if np.any(mask):
                                color=pg.intColor(j,hues=max(8,n_h),values=1,maxValue=220)
                                r1.plot(t[mask],rho_trace[mask,j],pen=pg.mkPen(color,width=max(1.0,lw*.8)),name=f"j={j+1}")
                        r1.addItem(pg.InfiniteLine(pos=1.0,angle=0,pen=pg.mkPen("#666666",width=1,style=Qt.DashLine)))
                        r1.setLabel("left","rho(A_y,j)")
                        r1.setLabel("bottom",rho_bottom_label)
                    rho_title="Sliding direct LNU: per-horizon rho(A_y,j) over retraining time"
                self.tab_titles[self.tabs.widget(2)].setText(rho_title + f" | prediction = {prediction_label}",color="#183b73",size="12pt",bold=True)
            else:
                rho_ay_values = np.asarray(z.get("rho_ay", []), dtype=float).reshape(-1)
                rho_x_ay = t[:rho_ay_values.size]
                model_value = np.asarray(z.get("model", z.get("config_honu", ["HONU"]))).reshape(-1)[0]
                if isinstance(model_value, bytes):
                    model_value = model_value.decode("utf-8", errors="replace")
                model_name = str(model_value).upper()
                nonlinear_local = model_name in {"QNU", "MLP"}
                rho_symbol = "rho(J_y(k))" if nonlinear_local else "rho(A_y)"
                mode_name = "Frozen" if result_mode == "mpc_frozen" else "Sliding-retraining"
                rho_title = f"{mode_name} {model_name} Plant local spectral radius {rho_symbol}"
                finite_ay = rho_ay_values[np.isfinite(rho_ay_values)]
                r0.setVisible(False)
                self.tab_titles[self.tabs.widget(2)].setText(rho_title + f" | prediction = {prediction_label}", color="#183b73", size="12pt", bold=True)
...
class MainWindow(QMainWindow):
    ...
    def _make_top_panel(self):
        ...
        for label, value in sorted(PHYSICAL_PLANT_MODELS.items(), key=lambda item: item[0].casefold()):
            self.physical_model_combo.addItem(label, value)
        ...
        for label, combo, width in (
            ("physical plant", self.physical_model_combo, 220),
            ("HONU plant", self.plant_model_combo, 90),
            ("controller", self.controller_model_combo, 90),
            ("plant learning", self.learning_combo, 125),
            ("controller learning", self.controller_learning_combo, 135),
            ("excitation u / reference d", self.reference_combo, 185),
        ):
            box = QWidget()
            v = QVBoxLayout(box)
        ...
        selector_layout.addWidget(self.btn_full_screen)
        ...
        self.mrac_plant_prediction_combo = QComboBox()
        self.mrac_plant_prediction_combo.addItem("One-step", "one_step")
        self.mrac_plant_prediction_combo.addItem("Recursive rollout", "recursive_rollout")
        self.mrac_plant_prediction_combo.setFixedWidth(150)
        self.mrac_plant_prediction_combo.setToolTip(
            "Plant HONU training used by MRAC. Recursive rollout starts every training segment from measured history and feeds back predicted y within the selected rollout length."
        )
        self.mrac_plant_rollout_length_spin = int_spin(minimum=2, maximum=200, width=100)
        self.mrac_plant_rollout_length_spin.setToolTip("Number of autoregressive rollout steps used to refine the MRAC plant HONU.")
        ...
        # Sampling and rendering are independent of the HONU architecture.
        # dt_sim controls the internal ODE integration resolution, while dt is
        # the sampled-data period used by identification and MRAC control.
        self.dt_sim_spin = double_spin(1.0e-6, 1000.0, 6, 0.001, 145)
        self.ode_solver_combo_mrac = QComboBox()
        for label, value in (("Auto (C++/Numba/SciPy)", "auto"), ("Native RK4 (C++/Numba)", "RK4"), ("Radau (SciPy)", "Radau"), ("BDF (SciPy)", "BDF"), ("LSODA (SciPy)", "LSODA"), ("RK45 (SciPy)", "RK45"), ("DOP853 (SciPy)", "DOP853")):
            self.ode_solver_combo_mrac.addItem(label, value)
        self.ode_solver_combo_mrac.setFixedWidth(205)
        self.ode_solver_combo_mrac.setToolTip("ODE solver used by physical-model data generation and MRAC testing. Auto selects the accelerated native RK4 path (C++/Numba) or a SciPy solver according to the model.")
        self.dt_spin = double_spin(1.0e-6, 1000.0, 6, 0.01, 145)
        ...
        self.dt_sim_spin.setToolTip("Internal maximum ODE integration step [s].")
        ...
        simulation_fields = (
            ("plant data duration [s]", self.t_end_spin),
            ("u step width [s]", self.step_hold_spin),
            ("tau_u [s]", self.tau_u_spin),
            ("dt MRAC [s]", self.dt_spin),
            ("dt_sim [s]", self.dt_sim_spin),
            ("ODE solver", self.ode_solver_combo_mrac),
            ("u_min", self.u_min_spin),
            ("u_max", self.u_max_spin),
            ("P regulator", self.preg_enabled_check),
            ("r_Preg", self.r_preg_spin),
            ("line width [px]", self.line_width_spin),
        )
        reference_fields = (
            ("controller/reference duration [s]", self.reference_duration_spin),
        ...
        plant_fields = (
            ...
            ("training", self.mrac_plant_prediction_combo),
            ("rollout N_r", self.mrac_plant_rollout_length_spin),
        ...
        def add_parameter_row(row, row_title, fields):
            ...
            for col, item in enumerate(fields, start=1):
                if item is None:
                    continue
                label, widget = item
                box = QWidget()
                ...
                lab.setToolTip(label)
                if widget is self.plant_epochs_spin:
                    self.plant_epochs_label = lab
                elif widget is self.mu_w_spin:
                    self.mu_w_label = lab
                elif widget is self.plant_lambda_spin:
                    self.plant_lambda_label = lab
                elif widget is self.controller_epochs_spin:
                    self.controller_epochs_label = lab
                v.addWidget(lab)
                v.addWidget(widget)
                grid.addWidget(box, row, col)

        add_parameter_row(0, "Simulation", simulation_fields)
        add_parameter_row(1, "Reference d", reference_fields)
        add_parameter_row(2, "Plant HONU", plant_fields)
        add_parameter_row(3, "Controller\nlearning", controller_learning_fields)

        self.reference_range_label = QLabel(
            "ODE test d hard limit: generate module 01 data first"
        )
        ...
        self.reference_range_label.setToolTip(
            "Module 04 constrains test d to min/max of measured y2 in data_uy.txt."
        )
        grid.addWidget(self.reference_range_label, 4, 1, 1, 7)

        for col in range(1, 8):
            grid.setColumnStretch(col, 1)
        outer.addWidget(parameters_group)

        self.physical_model_combo.currentIndexChanged.connect(self.on_physical_model_changed)
        ...
        self.learning_combo.currentIndexChanged.connect(self._update_mrac_learning_widget_state)
        ...
        self._update_mrac_learning_widget_state()
        return panel

    def _update_mrac_learning_widget_state(self, *_args):
        """Keep MRAC plant-learning widgets semantically consistent."""
        learning = str(self.learning_combo.currentData() or "batch")
        is_batch = learning == "batch"
        is_lm = learning == "LM"
        is_gradient = learning in ("GD", "NGD")

        self.plant_epochs_spin.setEnabled(not is_batch)
        self.mu_w_spin.setEnabled(is_gradient)
        self.plant_lambda_spin.setEnabled(is_batch or is_lm)

        if hasattr(self, "plant_epochs_label"):
            self.plant_epochs_label.setText("epochs")
            self.plant_epochs_label.setToolTip(
                "Not used by Batch / Ridge." if is_batch
                else ("Levenberg-Marquardt epochs." if is_lm else "Full passes over the plant-identification dataset.")
            )
        if hasattr(self, "mu_w_label"):
            self.mu_w_label.setText("mu_w")
            self.mu_w_label.setToolTip("Used only by GD/NGD plant identification.")
        if hasattr(self, "plant_lambda_label"):
            self.plant_lambda_label.setText("ridge lambda" if is_batch else ("lambda_0" if is_lm else "lambda"))
            self.plant_lambda_label.setToolTip(
                "Ridge regularization coefficient." if is_batch
                else ("Initial Levenberg-Marquardt damping." if is_lm else "Not used by GD/NGD.")
            )

        self.t_end_spin.setToolTip(
            "Duration of the physical ODE dataset generated by module 01. "
            "The number of identification samples is approximately duration / dt MRAC + 1."
        )
        self.reference_duration_spin.setToolTip(
            "Duration of reference d used for controller training (module 03) and physical ODE testing (module 04)."
        )
        self.controller_epochs_spin.setToolTip(
            "Number of controller-training epochs in module 03; independent of the plant-data duration."
        )
    ...
    def _make_left_panel(self):
        ...
        run_group = QGroupBox("Complete MRAC workflow - simulated plant")
        run_layout = QVBoxLayout(run_group)
        self.btn_run_data = QPushButton("1. Generate ODE simulation data")
        ...
        self.btn_run_evaluation = QPushButton("4. Test controller on physical ODE plant")
        self.btn_run_evaluation.clicked.connect(self.run_evaluation_from_metadata)
        ...
        run_layout.addWidget(self.btn_run_evaluation)
        ...
        self.btn_show_eval = QPushButton("ODE test")
    ...
    def load_metadata_from_setup(self):
        ...
        try:
            ...
            self.set_combo_value(self.mrac_plant_prediction_combo, read_setup_string("mrac_plant_prediction_training", "one_step"))
            self.mrac_plant_rollout_length_spin.setValue(read_setup_int("mrac_plant_rollout_length", 10))
            self.tau_u_spin.setValue(read_setup_float("tau_u", 0.5))
            self.dt_sim_spin.setValue(read_setup_float("dt_sim", 0.01))
            self.set_combo_value(self.ode_solver_combo_mrac, read_setup_string("ode_solver_method", "auto"))
    ...
    def update_selected_scripts_display(self):
        if not hasattr(self, "btn_run_data"):
            return
        script_01 = "01_generate_plant_data.py"
        script_02 = self.current_identification_script()
        script_03 = self.current_control_script()
        script_04 = self.current_evaluation_script()
        plant_model_file = self.selected_physical_model_value() + ".py"

        self.btn_run_data.setText("1. Generate ODE simulation data")
        ...
        self.btn_run_evaluation.setText("4. Test controller on physical ODE plant")
        ...
        self.btn_run_evaluation.setToolTip(script_04)
        self.physical_model_combo.setToolTip(f"plant_models/{plant_model_file}")
    ...
    def on_physical_model_changed(self, *_args):
        if getattr(self, "_loading_setup", False):
            return
        model_name = self.selected_physical_model_value()
        preset = recommended_preset(model_name) or {}
        widget_values = (
            (self.dt_sim_spin, "dt_sim"),
            (self.dt_spin, "dt"),
            (self.t_end_spin, "t_end"),
            (self.step_hold_spin, "step_hold_sec"),
            (self.reference_step_hold_spin, "reference_step_hold_sec"),
            (self.tau_u_spin, "tau_u"),
            (self.tau_d_spin, "tau_d"),
            (self.tau_1_spin, "Tau_1"),
            (self.tau_2_spin, "Tau_2"),
            (self.u_min_spin, "u_min"),
            (self.u_max_spin, "u_max"),
            (self.d_min_spin, "d_min"),
            (self.d_max_spin, "d_max"),
            (self.plant_n_y_spin, "plant_n_y"),
            (self.plant_n_u_spin, "plant_n_u"),
            (self.r_preg_spin, "r_preg"),
        )
        # A physical-model change starts a new configuration domain. Do not
        # restore cached LNU/QNU values belonging to the previously selected model.
        self.plant_parameter_cache.clear()
        self.controller_parameter_cache.clear()
        for widget, key in widget_values:
            if key in preset:
                widget.setValue(float(preset[key]))
        if "preg_blackbox_enabled" in preset:
            self.preg_enabled_check.setChecked(bool(preset["preg_blackbox_enabled"]))
        self.append_log(
            f"MODEL PRESET: {model_name}; physical u=[{self.u_min_spin.value():g}, {self.u_max_spin.value():g}], "
            f"physical d=[{self.d_min_spin.value():g}, {self.d_max_spin.value():g}]"
        )
        self.on_parameter_changed()
    ...
    def validate_setup_widgets(self):
        ...
        if self.dt_sim_spin.value() <= 0.0 or self.dt_spin.value() <= 0.0:
            raise ValueError("dt_sim and dt must be positive.")
        if self.dt_sim_spin.value() > self.dt_spin.value():
            raise ValueError("dt_sim must not be larger than the MRAC sampling period dt.")
        if self.t_end_spin.value() <= self.step_hold_spin.value():
            raise ValueError("t_sim must be larger than the plant-input step width.")
    ...
    def collect_setup_values(self):
        ...
        values = {
            ...
            "mrac_plant_prediction_training": str(self.combo_value(self.mrac_plant_prediction_combo)),
            "mrac_plant_rollout_length": int(self.mrac_plant_rollout_length_spin.value()),
            "tau_u": float(self.tau_u_spin.value()),
            "dt_sim": float(self.dt_sim_spin.value()),
            "ode_solver_method": str(self.combo_value(self.ode_solver_combo_mrac)),
    ...
    def apply_settings_to_setup(self, *_args, show_success=True, reload_plot=True, write_log=True):
        try:
            ...
            if write_log:
                ...
                f"dt_sim={self.dt_sim_spin.value():.8g} s, ODE solver={self.combo_value(self.ode_solver_combo_mrac)}, dt={self.dt_spin.value():.8g} s, "
    ...
    def active_parameter_summary(self):
        ...
        return (
            f"model={model_title}; {io_text}; dt_sim={self.dt_sim_spin.value():.6g} s; "
            f"dt_MRAC={self.dt_spin.value():.6g} s; normalization=3 sigma\n"
            ...
            f"prediction training={self.combo_value(self.mrac_plant_prediction_combo)}, rollout N_r={self.mrac_plant_rollout_length_spin.value()}\n"
    ...
    def validate_current_dataset_identity(self):
        """Ensure module-01 data belongs to the active physical configuration."""
        path = self.current_data_path()
        if not path.exists():
            raise FileNotFoundError("Current module-01 dataset is missing. Run module 01 first.")
        header_lines = []
        ...
        expected_model = self.selected_physical_model_value()
        if f"model_name={expected_model}" not in header:
            raise ValueError(
                f"Dataset {path.name} does not belong to active model {expected_model}. "
                "Run module 01 again after changing the physical model."
            )
        expected_dt = float(self.dt_spin.value())
        if f"dt={expected_dt:g} sec" not in header:
            raise ValueError(
                f"Dataset {path.name} was generated with another MRAC sampling period. "
                "Run module 01 again after changing dt."
            )
        expected_preg = bool(self.preg_mode_enabled())
        if f"preg_blackbox_enabled={expected_preg}" not in header:
            raise ValueError(
                f"Dataset {path.name} was generated with another P-regulated plant mode. "
                "Run module 01 again."
            )
    ...
    def run_generate_data(self):
        if not self.apply_metadata_to_setup():
            return
        self.set_active_output("01", load=False)
        self.log_step_request(1, "generate physical ODE training data", "data_uy.txt")
        self.run_script_file("01_generate_plant_data.py")
    ...
    def verify_training_plant_matches_current_dataset(self, plant_model):
        ...
        dataset_stats = load_stats(BASE_DIR / "simulated_normalization.npz")
    ...
    def run_controller_from_metadata(self):
        ...
        try:
            ...
            self.validate_current_dataset_identity()
            training_dataset = BASE_DIR / "data_uy_normalized.txt"
    ...
    def run_evaluation_from_metadata(self):
        ...
        try:
            ...
            self.append_log(f"RUN BLOCKED: {exc}")
            return
        self.set_active_output("04eval", load=False)
        self.log_step_request(4, "test controller on physical ODE plant", self.current_eval_output_file())
        self.run_script_file(self.current_evaluation_script())
    ...
    def set_run_buttons_enabled(self, enabled):
        ...
        self.btn_run_evaluation.setEnabled(enabled)

    def expected_outputs_for_script(self, script_name):
        """Return files that must be freshly produced by a successful module run."""
        if script_name == "01_generate_plant_data.py":
            return [BASE_DIR / "data_uy.txt", BASE_DIR / "data_uy_normalized.txt", BASE_DIR / "simulated_normalization.npz"]
        ...
        if script_name.startswith("04_test_ODE_plant_"):
    ...
    def capture_effective_run_metadata(self):
        ...
        return {
            ...
            "dt_sim": float(read_setup_float("dt_sim", float(self.dt_sim_spin.value()))),
    ...
    def on_process_finished(self, exit_code, exit_status):
        ...
        self.refresh_file_list()
        if finished_script == "01_generate_plant_data.py":
    ...
    def open_current_graph_full_screen(self):
        ...
        if output_kind == "01":
            ...
            metadata_text = " | ".join(metadata_parts) + preg_suffix
            window_title = f"Physical ODE test | {graph_title}"
        else:
            metadata_parts = [plant_name]
            if parameter_text:
                metadata_parts.append(parameter_text)
    ...
    def plant_identification_series_length(self):
        data_path = BASE_DIR / "data_uy_normalized.txt"
    ...
    def compute_identification_fit(self, bibs_file_name):
        ...
        stats = load_stats(BASE_DIR / "simulated_normalization.npz")
    ...
    def plot_output_bundle(self, file_name, columns, data, dt_txt):
        ...
        if file_name.startswith("bibs_plant"):
            ...
            self.add_graph_tab("04 physical ODE test")
            self.plot_data(columns, data, file_name, dt_txt)
        else:
    ...
    def refresh_file_list(self):
        ...
        active_outputs = (
            ...
            ("04 active physical ODE test", self.current_eval_output_file()),
        )
        self.output_combo.blockSignals(True)
    ...
    def next_step_hint_for_file(self, file_name):
        ...
        if file_name.startswith("eval_"):
            return "Run modules 01, 02, and 03, then module 04 physical ODE testing."
    ...
    def show_missing_output_state(self, file_name, detail=""):
        ...
        if file_name.startswith("bibs_plant"):
            ...
            self.show_message_tab("04 physical ODE test", f"{eval_name}\nnot generated yet\n\n{hint}")
        else:
    ...
    def plot_plant_data(self, columns, data, t, dt_txt):
        ...
        if "dt_sim" in run_meta:
            xlabel = f"t [sec], dt_sim={float(run_meta['dt_sim']):.8g} [sec]"
        else:
    ...
    def plot_eval_data(self, columns, data, t, file_name, dt_txt):
        ...
        title = (
            physical_title + "\n" +
            f"Module 04 physical ODE validation: controller trained with "
