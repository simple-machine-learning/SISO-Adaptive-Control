...
try:
    from common.physical_native import (
        native_enabled as physical_native_enabled,
        native_import_error as physical_native_import_error,
        strict_native as physical_native_strict,
        simulate_interval as physical_native_simulate_interval,
    )
except ImportError as exc:
    _PHYSICAL_NATIVE_IMPORT_ERROR = f"{type(exc).__name__}: {exc}"
    def physical_native_enabled(): return False
    def physical_native_import_error(): return _PHYSICAL_NATIVE_IMPORT_ERROR
    def physical_native_strict(): return False
    def physical_native_simulate_interval(*args, **kwargs):
        raise RuntimeError("C++ physical ODE backend import failed")

_NATIVE_ODE_DISABLED_MODELS = set()
...
def _selected_ode_method(par, solver):
    ...
    if requested == "auto":
        return "Radau" if str(par.plant_model_name) in _STIFF_ODE_MODELS else "native_rk4"
    if requested in {"native", "native_rk4", "rk4", "compiled"}:
        return "native_rk4"
...
def _adaptive_interval(chi, u_command, dt_sample, par, solver, preg=False, r_preg=1.0):
    ...
    if method == "native_rk4":
        raise RuntimeError("Internal error: adaptive interval requested for native RK4")
...
def _native_required(chi, u_command, dt_sample, par, solver, preg=False, r_preg=1.0):
    """Run the selected ODE backend without a silent fallback."""
    method = _selected_ode_method(par, solver)
    if method != "native_rk4":
        return _adaptive_interval(
            chi, u_command, dt_sample, par, solver, preg=preg, r_preg=r_preg
        )

    model_name = str(par.plant_model_name)
    if model_name in _NATIVE_ODE_DISABLED_MODELS:
        raise RuntimeError(
            f"C++/Cython ODE backend was disabled after an earlier failure for model '{model_name}'."
        )
    if not physical_native_enabled():
        detail = physical_native_import_error()
        env_value = os.environ.get("SISO_ODE_NATIVE", "1")
        raise RuntimeError(
            "C++/Cython physical ODE backend is required but unavailable or disabled. "
            f"Python={sys.executable}; SISO_ODE_NATIVE={env_value!r}; "
            f"import_error={detail or 'none (backend may be disabled)'}. "
            "Activate the project environment and run ./build_native_linux.sh."
        )
    model_module = _model_from_par(par)
    try:
        return physical_native_simulate_interval(
            chi, u_command, dt_sample, _internal_step(dt_sample, solver),
            model_module, par, preg=preg, r_preg=r_preg,
            output_function=controlled_output if preg else None,
        )
    except (RuntimeError, ValueError, OverflowError, FloatingPointError) as exc:
        _NATIVE_ODE_DISABLED_MODELS.add(model_name)
        raise RuntimeError(
            f"C++/Cython physical ODE backend failed for model '{model_name}'. "
            "No silent solver fallback was used."
        ) from exc

def simulate_sample_period_zoh(chi, u_const, dt_sample, par, solver):
    """Integrate one ZOH interval using the mandatory compiled backend."""
    native_result = _native_required(chi, u_const, dt_sample, par, solver)
    return native_result[0]


def simulate_sample_period_preg(chi, u_new_const, dt_sample, par, solver, r_preg):
    """Integrate one interval with continuous inner P feedback in compiled code."""
    return _native_required(
