...
from project_setup import (
    ...
    dt as dt_MRAC,
    controller_lnu_plant_file as plant_file,
    ctrl_lnu_qnu_file as ctrl_file,
    ...
    bibs_ctrl_lnu_qnu_file as bibs_ctrl_file,
    reference_type,
    reference_seed,
    d_min,
    d_max,
...
def closed_loop_characteristic_metrics(w, v, r_0_value, xi_operating, n_y, n_u, n_u1, n_M):
    # Plant: y(k) = w_y*[y(k-1)...] + w_u*[u(k-n_u1-1)...]
    # QNU controller is locally linearized at xi_operating:
    # q ~= q0 + grad_xi_q^T (xi - xi_operating).
    # Homogeneous stability test uses d=0 and y_ref=0, therefore e_ref=y.
    # Hence v_cl contains local effective feedback coefficients from y to q.
    w_y = w[1:1+n_y]
    w_u = w[1+n_y:1+n_y+n_u]

    _, J_phi = qnu_phi_and_jacobian(xi_operating)
    grad_xi_q = v @ J_phi
    v_y_local = grad_xi_q[1:1+n_y]
    v_e_local = grad_xi_q[1+n_y:1+n_y+n_y]
    v_cl = v_y_local + v_e_local
    ...
    for j in range(n_u):
        for p in range(n_y):
            r = n_u1 + (j + 1) + p
            if r < n_M:
                m[r] = m[r] - r_0_value*w_u[j]*v_cl[p]
...
# =============================================================================
# Module 03: controller training on LNU HONU plant
# =============================================================================
...
for epoch in range(epochs):
    ...
    for k in range(N_start, N):
        ...
        # ---------------------------------------------------------------------
        # LNU plant
        # y(k) = w_y * [y(k-1), ..., y(k-n_y)] +
        #        w_u * [u(k-n_u1-1), ..., u(k-n_u1-n_u)]
        # ---------------------------------------------------------------------

        x[0] = 1.0
        x[1:1+n_y] = y[k-n_y:k][::-1]
        x[1+n_y:] = u[k-n_u1-n_u:k-n_u1][::-1]

        y[k] = w @ x

        dxdv[1:1+n_y, :] = dydv[k-n_y:k, :][::-1]
        dxdv[1+n_y:, :] = dudv[k-n_u1-n_u:k-n_u1, :][::-1]
        dydv[k, :] = w @ dxdv

        dxdr_0[1:1+n_y] = dydr_0[k-n_y:k][::-1]
        dxdr_0[1+n_y:] = dudr_0[k-n_u1-n_u:k-n_u1][::-1]
        dydr_0[k] = w @ dxdr_0
        ...
        # Stable GD/NGD evaluation.  The direct expressions g@g, g_r_0**2
        # and I-eta*g*g^T overflow for large QNU sensitivities even when the
        # normalized update itself is finite.  Use scaled norms and the exact
        # rank-one eigenvalues instead; this does not clip u or alter the
        # mathematical GD/NGD update.
        ...
        # Exact 2-norm and spectral radius of I-eta*g*g^T.  Its eigenvalues
        # are 1 (multiplicity n_v-1) and 1-eta*||g||^2, so no SVD or outer
        # product is required.
        rank_one_eig = 1.0 - eta_g2_v
        A2_v_all[k_all] = builtins.max(1.0, abs(rank_one_eig))
        Rho_v_all[k_all] = builtins.max(1.0, abs(rank_one_eig))
        ...
        m_norm_all[k_all], Rho_all[k_all] = closed_loop_characteristic_metrics(
            w, v, r_0_value, xi, n_y, n_u, n_u1, n_M
        )
    ...
    print(f"Controller LNU-QNU epoch {epoch+1}/{epochs}: RMSE={rmse:.6g}, r_0={r_0_value:.6g}", flush=True)
...
fig.suptitle(
    "Measured dataset" + "\n" + r"LNU plant + QNU controller, learning=" + learning +
    r", $\mu_v$=" + str(mu_v) +
